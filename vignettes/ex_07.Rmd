...
(f) But is the model appropriate for the data. 
Run a posterior predictive checknad see what is going on.

```{r ex_pp_check, message = F}
...
(g) What is wrong with the fit? 
Why could it look like the way it does? 
How can you fix it?

```{r ex_refit, message = F}
...
<!-- (g) The model does not seem to capture a bimodality very well. 
This bimodality is likely from two different types of speakers, those that identified themselves as females vs. those that identified themselves as male. We should add gender as a predictor.
...
pp_check(ex_model_refit, nsamples = 100)
-->
